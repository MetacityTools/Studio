...
import { GeometryData, MaterialData } from '../Data';
...
export class ItemsMap {
    ...
    getSubsetID(modelID: number, material?: MaterialData, customID = 'DEFAULT') {
    ...
    private newItemsMap(modelID: number, geometry: GeometryData) {
        const startIndices = geometry.index as Uint32Array;
        this.map[modelID] = {
            indexCache: startIndices.slice(0, geometry.index.length),
            map: new Map(),
        };
        return this.map[modelID] as Items;
    }

    private fillItemsWithGroupInfo(group: Group, geometry: GeometryData, items: Items) {
        let prevExpressID = -1;
        ...
        for (let i = materialStart; i <= materialEnd; i++) {
            const index = geometry.index[i];
            const bufferAttr = geometry.expressID;
            const expressID = bufferAttr[index];
