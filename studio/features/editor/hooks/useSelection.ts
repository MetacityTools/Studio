...
function caseToggleSelection(
  ...
  for (const [oldModel, oldSubmodelIDs] of oldSelection) {
    ...
    if (!newSubmodelIDs) {
      ...
      //make a diff
      const submodelIDs = caseSameModelToggleUpdate(
        oldModel,
        oldSubmodelIDs,
        newSubmodelIDs,
      );
      if (submodelIDs.length > 0)
        combinedSelection.set(oldModel, new Set(submodelIDs));
  ...
  for (const [newModel, newModelSelection] of newSelection) {
    ...
    if (!found)
      combinedSelection.set(
        newModel,
        caseOnlyNewModelUpdate(newModel, newModelSelection),
      );
  }
...
function caseExtendSelection(
  ...
  for (const [newModel, newSubmodelIDs] of newSelection) {
    ...
    if (!oldSubmodelIDs) {
      ...
      //extend
      const submodelIDs = caseSameModelExtendUpdate(
        newModel,
        oldSubmodelIDs,
        newSubmodelIDs,
      );
