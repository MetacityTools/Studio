...
export async function parse(model: UserInputModel): Promise<ModelData> {
  ...
  const { buffers, submodelIdx, coordType, metadata, type } =
    getBuffersAndMeta(features);
...
function getBuffersAndMeta(features: FeatureCollection) {
  ...
  let type: PrimitiveType | undefined = getType(
    features.features[0]?.geom.type,
  );
