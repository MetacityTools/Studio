...
export async function load(files: Map<string, Blob>, updateStatus?: (status: string) => void) {
...
async function filterMetacity(files: Map<string, Blob>): Promise<UserInputModel[]> {
  const data = [];
...
async function filterShapefile(files: Map<string, Blob>): Promise<UserInputModel[]> {
  const data = [];
  for (const [name, file] of files) {
    ...
    pool.process(
      ...
      (data) => {
        if (data) {
          ...
          updateStatus && updateStatus(`Loading models: Could not parse model ${model.name}`);
        }
      },
      worker
    );
