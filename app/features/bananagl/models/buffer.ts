...
export class Buffer {
  ...
  update(gl: WebGL2RenderingContext) {
    ...
    for (const [start, end] of this.partsToUpdate_) {
      gl.bufferSubData(gl.ARRAY_BUFFER, start * this.data.BYTES_PER_ELEMENT, this.data.subarray(start, end));
    }
  ...
  getView(typeConstructor: ConstructableTypedArray) {
    return new typeConstructor(this.data.buffer as ArrayBuffer);
  }
  ...
  applyMatrix(matrix: mat2 | mat3 | mat4, size: number) {
    if (matrix.length === 4 && size === 2) {
      ...
      throw new Error(`Invalid combination of matrix (${matrix.length}) and element (${size}) size.`);
    }
  }
  ...
  private applyMatrix2(matrix: mat2) {
    for (let i = 0; i < this.data.length; i += 2) {
      vec2.transformMat2(this.data.subarray(i, i + 2) as vec2, this.data.subarray(i, i + 2) as vec2, matrix);
    }
  }

  private applyMatrix3(matrix: mat3) {
    for (let i = 0; i < this.data.length; i += 3) {
      vec3.transformMat3(this.data.subarray(i, i + 3) as vec3, this.data.subarray(i, i + 3) as vec3, matrix);
    }
  }
  ...
  private applyMatrix4(matrix: mat4, size: number) {
    for (let i = 0; i < this.data.length; i += size) {
      if (size === 3) {
        vec3.transformMat4(this.data.subarray(i, i + 3) as vec3, this.data.subarray(i, i + 3) as vec3, matrix);
      } else if (size === 4) {
        vec4.transformMat4(this.data.subarray(i, i + 4) as vec4, this.data.subarray(i, i + 4) as vec4, matrix);
      }
    }
  }
